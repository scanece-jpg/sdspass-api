...
Aquatic Hesap Kuralları (SEA/CLP Annex I Tablo 4.1.2 — Karışımlar):
  CLP eşiği %25; Ci değerleri /100 kesire çevrildiğinden eşik 0.25 kullanılır.
  0.1%/M ve 1%/M değerleri dahil etme (inclusion) cut-off'larıdır — sınıflandırma eşiği değil.
  H400: Σ(Ci × M_acute)/100                              ≥ 0.25 (= %25)
  H410: Σ(Ci × M_chr)[Kronik1]/100                       ≥ 0.25 (= %25)
  H411: (10×Σ(Ci×M)[K1] + Σ(Ci)[K2])/100                ≥ 0.25
  H412: (100×Σ(Ci×M)[K1] + 10×Σ(Ci)[K2] + Σ(Ci)[K3])/100 ≥ 0.25
  H413: Σ(Ci)[tüm kronik]/100                             ≥ 0.25
...
def calculate_aquatic(
    ...
    # Kaynak: CLP Annex I §4.1.3.5.5
    #
    # CLP formülü (Ci yüzde olarak):  Σ(Ci × Mi) ≥ 25
    # Kod, Ci/100 kesire çevirdiğinden eşik de dönüştürülür: 25/100 = 0.25
    # Yani doğru eşik: 0.25  (0.0025 DEĞİL — o 100× çok küçüktür)
    #
    ...
    # H400: Σ(Ci × M_akut)/100 ≥ 0.25  ↔  Σ(Ci × Mi) ≥ %25
    if sum_acute_m >= 0.25:
        return AquaticResult(
